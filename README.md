...
其中 `physical=[vx, vy, yaw_rate, 4 个轮速]`，`dt=0.02`。
...
```text
[vx, vy, yaw_rate, front_right_wheel_speed, front_left_wheel_speed, back_right_wheel_speed, back_left_wheel_speed]
```
...
Mecanum 新训练默认使用 `s_dot=f(s,a)` 的 `obs_derivative` 模式；旧的 3 维、7 维 latent 或 11 维 mecanum checkpoint 不能直接继续使用，需要重新训练。
